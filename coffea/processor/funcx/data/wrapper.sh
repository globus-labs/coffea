...
*.opensciencegrid.org:pubkey=<BUILTIN-opensciencegrid.org.pub>,url=http://oasis-replica.opensciencegrid.org:8000/cvmfs/*;http://cvmfs.fnal.gov:8000/cvmfs/*;http://cvmfs.racf.bnl.gov:8000/cvmfs/*'
	...
	if [ -n "$(ldd parrot_run 2>&1 | grep 'not found')" ]; then
		log "ldd" "linkage of parrot" ldd parrot_run
		exit 169
	else
		log "parrot OK"
	fi
	...
	exec parrot_run -M /cvmfs/cms.cern.ch/SITECONF/local=$PWD/siteconf -M /sbin/ifconfig=/bin/echo -t "$PARROT_CACHE/ex_parrot_$(whoami)" bash $0 "$*"
...
log "top" "machine load" top -b\|head -n 50
